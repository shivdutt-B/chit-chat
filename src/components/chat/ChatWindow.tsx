...
export const ChatWindow: React.FC<ChatWindowProps> = ({ messages, onSendMessage, chatName, chatAvatar }) => {
    ...
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 bg-card border-b border-border">
        <div className="flex items-center">
          ...
          <div className="ml-3">
            <h2 className="text-lg font-semibold text-card-foreground font-heading">{chatName}</h2>
        ...
        {messages.map((message) => (
          <div key={message.id} className={`flex ${message.isOwn ? "justify-end" : "justify-start"}`}>
            ...
            >
              ...
              >
                {!message.isOwn && (
                  <div className="text-xs font-medium text-gray-500 mb-1">{message.sender}</div>
                )}
                <div className="text-sm leading-relaxed whitespace-pre-wrap">{message.content}</div>
                <div
                  className={`text-xs mt-1 ${message.isOwn ? "text-primary-100" : "text-gray-400"}`}
                >
      ...
      {/* Smart Replies */}
      {mockSmartReplies.length > 0 && (
        <div className="flex-shrink-0 px-6 py-2">
          <div className="flex space-x-2 overflow-x-auto scrollbar-hide pb-2">
            {mockSmartReplies.map((reply: SmartReply) => (
              <button
                key={reply.id}
                className="flex-shrink-0 px-4 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-200 hover:bg-gray-50 rounded-full transition-colors duration-200"
      ...
      {/* Input Container */}
      <div className="flex-shrink-0 px-6 py-4 bg-card border-t border-border">
        ...
        <>
          ...
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-popover rounded-2xl shadow-2xl max-w-2xl w-[90%] max-h-[80vh] overflow-hidden z-50 border border-border">
            <div className="px-6 py-4 border-b border-border">
              <h3 className="text-xl font-semibold text-popover-foreground font-heading">Conversation Summary</h3>
            </div>
